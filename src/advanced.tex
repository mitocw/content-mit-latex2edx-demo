...
Valid answers include {\large \texttt{X*Y}} and {\large \texttt{i*Z}}.  But {\large \texttt{Y*X}} is properly graded as being invalid.

Note that {\large \texttt{X * exp(-i*Y) * Y * exp(i*Y)}} also works (matrix
exponentials do the correct thing --- although, unfortunately, the edX
edge system doesn't provide {\large \texttt{scipy.linalg}}, and so it will
not work there).
...
\edXabox{expect="X*Y" 
  ...
  preprocessorClassName="MathjaxPreprocessorForQM"
  preprocessorSrc="/static/js/mathjax_preprocessor_for_QM.js"
 }%
...
the course \texttt{/python_lib} or \texttt{/code} directory.  This means that each
...
Unfortunately, the edX Studio system is broken, in that it does
not allow use of \texttt{/python_lib} or \texttt{/code} directories of python
code libraries.  Thus, for this demo to work on the edx.org
Studio system, we are forced to paste all the python code
...
Type in \texttt{(2*|0>+3*|b>)/sqrt(13)} as the answer below.

Multiple correct answer are supported.  \texttt{|b>} is also correct, in
this example.

{\color{red}{\textbf{Remember to explicitly denote multiplication using \texttt{*}}}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{edXsequential}{Adaptive hints}
...
\begin{edXsequential}{Drag and drop input}
...
\begin{edXsequential}{Graphical input using javascript}
